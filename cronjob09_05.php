<?php
    ...
    mysqli_query($link, $sqlSelectDate);

    $sql = "SELECT * FROM hashtags WHERE enumStatus='N' ORDER BY intID ASC LIMIT 0, 70";
    ...
    if(mysqli_num_rows($result) < 1)
    {
        ...
        if(mysqli_num_rows($result) < 1)
        {
            $sql = "SELECT * FROM hashtags ORDER BY intDailyPostCount ASC LIMIT 0, 70";
            $result = mysqli_query($link, $sql);
        }
    }
    ...
    while($data = mysqli_fetch_array($result))
    {
        ...
        $fetchRow = mysqli_fetch_row($tempResult);
        $sum = $fetchRow[0];

        $sql = "SELECT COUNT(intID) AS cnt FROM hashtags_log WHERE intHashTagID='" . $intId . "'";
        $tempResult = mysqli_query($link, $sql);
        $fetchRow = mysqli_fetch_row($tempResult);
        $tempCnt = $fetchRow[0];
        ...
        $avgPost = $post / count($postlikemax);
        $avgLike = $sum / 9;
        $intMinPostLikePerHour = min($postlikemax);

        $sqlUpdate = "UPDATE hashtags SET intPostCount='".$postcount."' WHERE intID='".$intId."'";
        mysqli_query($link, $sqlUpdate);
        // $sqlUpdate = "UPDATE hashtags  SET intPostCount='".$postcount."', intDailyPostCount='".$dailypostcount
        ...
        // if($num > 0)
        // {
        //     $res = mysqli_fetch_row($resultCount);
        //     $id = $res['intID'];
        //     $sqlInsertLog = "UPDATE hashtags_log SET intPostCount='".$postcount."', intDailyPostCount='".$dailypostcount."', intDAPC='".$DAPC."', dt_Date=now() WHERE intID='".$id."'";
        //     mysqli_query($link,$sqlInsertLog);
        // }else{
        //     $sqlInsertLog="INSERT INTO hashtags_log SET intHashTagID='".$intId."', intPostCount='".$postcount."', intDailyPostCount='".$dailypostcount."', intDAPC='".$DAPC."', dt_Date=now()";
        //     mysqli_query($link,$sqlInsertLog);
        // }
        $update="UPDATE hashtags SET enumStatus='Y', dt_CronRunDate=now() WHERE intID='".$intId."'";
