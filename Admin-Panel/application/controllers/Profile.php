...
class Profile extends CI_Controller {
    ...
    public function index()
    {
        ...
        if ($this->Admin_model->verifyUser()) {
            $sql = "SELECT * FROM admin WHERE id = " . $this->session->userdata("admin_id");
        ...
        if ($this->Admin_model->verifyUser()) {
            if ($this->input->post()) {
                ...
                if(isset($_FILES['file'])){
                    ...
                    $target = "assets/images/users/admin_" . $this->session->userdata("admin_id") . "." . $file_ext;
                    $bl = move_uploaded_file($file_tmp, $target);
                }
                $email = $postData['email'];
                $name = $postData['name'];
                $sql = "UPDATE admin SET username = '$name', email = '$email' WHERE id = " . $this->session->userdata("admin_id");
                $this->db->query($sql);
